...
/*
    var _geocoder;
    function _addMarker(entry_id, address, icon) {
        // TODO: cache the geolocation's or write them to database during place creation
        _geocoder.getLatLng(
                address,
                function(point) {
                    if (!point) {
//                        alert(address + " not found");
                        ...
                        kk.kultur.get_map().addOverlay(marker);
                    }
                }
                );
    }
    ...
    var public_api = {
		...
		if ($('places') != null) {
                ...
                if (allObjects != null && allObjects.length==1) {
                    ...
                    _map.setCenter(new GLatLng(52.528000, 13.330000), $('map').getWidth() < 500 ? 14 : 15);
                    } else {
        ...
        addAllMarkerOfSpecifiedType2: function(allObjects) {
            ...
            allObjects.each(function(entry, index) {
                try {
                    ...
                    var desc = entry.select('.o_description')[0].innerHTML;
                    if (desc==null || desc.blank()) {
                        desc = "keine weitere Beschreibung";
                    }
                    ...
                    var markerHtml = "<div class='marker_info'>"
                            + "<h4>" + title + ""
                            //+ "<ul class='stars' style='float:right;margin-right:10px;'><li class='current_rating' style='width:" + rating + "'>" + rating + "</li></ul>"
                            + "</h4>"
                            //+ "<div>" + tags + "</div><hr/>"
                            + "<div>" + desc + "</div>"
                            //+ "<p>" + address + "</p>"
